...
class DossierManager {
	...
	public function findOneByFormation($ine, $codeFormation) {
		$rs = $this->db->prepare("SELECT * FROM `DOSSIER` WHERE `INE` = ? AND CODE_FORMATION` = ?;");
	...
	public function update(Dossier $dossier) {
		return $this->db->prepare("UPDATE `DOSSIER` SET `AUTRE` = ?, `NOM` = ?, `PRENOM` = ?, `ADRESSE` = ?, `COMPLEMENT` = ?, `CODE_POSTAL` = ?, `VILLE` = ?, `DATE_NAISSANCE` = ?, `LIEU_NAISSANCE` = ?, `FIXE` = ?, `PORTABLE` = ?, `MAIL` = ?, `LANGUES` = ?, `NATIONALITE` = ?, `SERIE_BAC` = ?, `ANNEE_BAC` = ?, `ETABLISSEMENT_BAC` = ?, `DEPARTEMENT_BAC` = ?, `PAYS_BAC` = ?, `ACTIVITE` = ?, `TITULAIRE` = ?, `EMPLOI` = ?, `INFORMATIONS` = ?, `DATE_DOSSIER` = ? WHERE `INE` = ? AND `CODE_FORMATION` = ?;")
						->execute(array(
							$dossier->getAutre(),
							$dossier->getNom(),
							$dossier->getPrenom(),
